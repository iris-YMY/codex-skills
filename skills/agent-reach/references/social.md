...
## TikHub 深度数据路由（小红书 / 抖音 / 快手）

TikHub 是付费的深度数据后端。只在以下条件之一成立时使用：

- 用户明确指定 TikHub；
- 用户要求深度、批量、结构化、跨样本或评论数据研究；
- 抖音或快手任务需要搜索、详情、评论或用户数据；
- 小红书的 OpenCLI 路径不可用，且用户要求继续使用付费 API。

普通小红书浏览、单条阅读或已有浏览器登录态任务继续优先使用 OpenCLI。一次任务只选择一个主后端；不要同时调用 OpenCLI 和 TikHub 获取相同数据。TikHub 的业务接口可能计费，只调用完成用户请求所需的最少接口和最小页数，不做探索性批量调用。

优先使用以下只读工具，不调用注册设备、增加播放量、广告、DouPlus、发布、互动或其他写入型接口：

| 平台 | 意图 | TikHub 工具 |
|---|---|---|
| 小红书 | 搜索笔记 | `tikhub_xiaohongshu.xiaohongshu_app_v2_search_notes` |
| 小红书 | 笔记详情 | `tikhub_xiaohongshu.xiaohongshu_web_v3_fetch_note_detail` |
| 小红书 | 评论 | `tikhub_xiaohongshu.xiaohongshu_app_v2_get_note_comments` |
| 小红书 | 用户 | `tikhub_xiaohongshu.xiaohongshu_app_v2_get_user_info` |
| 抖音 | 搜索视频 | `tikhub_douyin.douyin_search_fetch_video_search_v2` |
| 抖音 | 分享链接详情 | `tikhub_douyin.douyin_web_fetch_one_video_by_share_url` |
| 抖音 | 评论 | `tikhub_douyin.douyin_web_fetch_video_comments` |
| 抖音 | 用户 | `tikhub_douyin.douyin_web_fetch_user_profile_by_uid` |
| 快手 | 搜索视频 | `tikhub_kuaishou.kuaishou_app_search_video_v2` |
| 快手 | 分享链接详情 | `tikhub_kuaishou.kuaishou_app_fetch_one_video_by_url` |
| 快手 | 评论 | `tikhub_kuaishou.kuaishou_app_fetch_video_comment` |
| 快手 | 用户 | `tikhub_kuaishou.kuaishou_app_fetch_one_user_v2` |

调用前声明平台和 TikHub 后端。搜索先取第一页；只有用户要求更多或第一页不足时再翻页。结果中保留请求来源、平台、接口意图、分页参数和返回时间；不得记录或输出 API Key。
